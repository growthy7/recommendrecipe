...
export default function FridgeTab() {
  ...
  const loadFromKV = async () => {
    ...
    try {
      const res = await fetch(`/api/fridge/${FAMILY_CODE}`);
  ...
  const saveToKV = useCallback(async (items: string[]) => {
    setSyncStatus("syncing");
    try {
      const res = await fetch(`/api/fridge/${FAMILY_CODE}`, {
        method: "PUT",
  ...
  const updateFridge = (items: string[]) => {
    ...
    debounceRef.current = setTimeout(() => saveToKV(items), 1000);
  };
      ...
      <section className="bg-white rounded-2xl p-4 shadow-sm">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="ml-auto text-xs text-gray-400">
            ...
            {syncStatus === "error" && "⚠️ 오프라인"}
            {fridge.length > 0 && syncStatus === "idle" && `${fridge.length}개`}
          </span>
        </div>
        {fridge.length === 0 ? (
